...
class FIXED_Magic{
    ...
    public:
        Bitboard attacks_table[88772];
        FIXED_Magic();
        ~FIXED_Magic() = default;

        void init(Magic *magic_init, Bitboard *attacks[], Bitboard magics[], Bitboard masks[], int dir[][2], bool is_bishop);
        void generate_magic_sliders();
        constexpr void generate_magic_knights();
        constexpr void generate_magic_kings();

        int magic_index_bishop(int sq, Bitboard occ);
        int magic_index_rook(int sq, Bitboard occ);
